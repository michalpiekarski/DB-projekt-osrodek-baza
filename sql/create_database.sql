...
ALTER TABLE KLIENCI ADD CONSTRAINT ImieNazwiskoAdres UNIQUE ( imie , nazwisko , ulica , mieszkanie , kod_pocztowy , miasto ) ;
...
ALTER TABLE OBIEKTY ADD CONSTRAINT OsrodekBudynekNumer UNIQUE ( osrodek , budynek , numer ) ;
...
ALTER TABLE OSRODKI ADD CONSTRAINT Adres UNIQUE ( ulica , kod_pocztowy , miasto ) ;
...
ALTER TABLE PRACOWNICY ADD CONSTRAINT ImieNazwiskoAdres UNIQUE ( imie , nazwisko , ulica , mieszkanie , kod_pocztowy , miasto ) ;
...
ALTER TABLE ZAMOWIENIA_POSILKOW ADD CONSTRAINT RachunekTypData UNIQUE ( rachunek , typ , data ) ;
...
ALTER TABLE ZAMOWIENIA_USLUG ADD CONSTRAINT RachunekTypData UNIQUE ( rachunek , typ , data ) ;
...
ALTER TABLE ZAMOWIENIA_WYPOZYCZEN ADD CONSTRAINT RachunekTypData UNIQUE ( rachunek , typ , data_od , data_do ) ;
...
CREATE OR REPLACE TRIGGER KlientID_AI BEFORE
  INSERT ON PRACOWNICY FOR EACH ROW BEGIN :NEW.id := KlientID.NEXTVAL;
END;
/

CREATE SEQUENCE RachunekID START WITH 0 NOCACHE ORDER ;
CREATE OR REPLACE TRIGGER RachunekID_AI BEFORE
  INSERT ON RACHUNKI FOR EACH ROW BEGIN :NEW.id := RachunekID.NEXTVAL;
END;
/

CREATE SEQUENCE RezerwacjaID START WITH 0 NOCACHE ORDER ;
CREATE OR REPLACE TRIGGER RezerwacjaID_AI BEFORE
  INSERT ON REZERWACJE FOR EACH ROW BEGIN :NEW.id := RezerwacjaID.NEXTVAL;
END;
/

CREATE SEQUENCE ZamowieniePosilkuID START WITH 0 NOCACHE ORDER ;
...
CREATE SEQUENCE ZamowienieUslugiID START WITH 0 NOCACHE ORDER ;
...
CREATE SEQUENCE ZamowienieWypozyczeniaID START WITH 0 NOCACHE ORDER ;
